...
var ProductCart = function () {
  ...
  _createClass(ProductCart, [{
    ...
    value: function loadProducts() {
      ...
      var productsHtml = this.products.reduce(function (html, product, index) {
        return html += _this.generateProductHtml(product, index);
      }, '');
    ...
    key: 'totalPricecalc',
    value: function totalPricecalc() {
      var total_sum_blocks = $('.total_price');
      var sum = 0;
      $.each(total_sum_blocks, function (index, value) {
        var prices = Number($(total_sum_blocks[index]).text());
        sum += prices;
      });
    ...
    key: 'calculate_price',
    value: function calculate_price(index) {
      var mi = this;
      $('.products_in_cart').change(function (e) {
        var current = e.target;
        var current_block = e.target.parentElement.parentElement;
        var currentId = $(current_block).find('.table_product_id').text();
        var currentQuantity = $(current_block).find('.products_in_cart').val();
        ...
        mi.totalPricecalc();
        var a = mi.findProductToUpdate(currentId);
        mi.updateCart(a, currentQuantity);
        // console.log(mi.products);
      });
    }
  }, {
    key: 'findProductToUpdate',
    value: function findProductToUpdate(currentId) {
    ...
    key: 'updateCart',
    value: function updateCart(a, currentQuantity) {
      this.products[a].product_quantity = currentQuantity;
      localStorage.setItem('PRODUCTS', JSON.stringify(this.products));
    }
  }, {
    key: 'generateProductHtml',
    value: function generateProductHtml(product, index, e) {
